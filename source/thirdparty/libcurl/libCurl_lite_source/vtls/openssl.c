...
//
// openssl detailed log
//

struct SessionHandle *bio_data = NULL;
...
void apps_ssl_info_callback(const SSL *s, int where, int ret)
{
	...
	else if (where & SSL_CB_ALERT)
	{
		...
		infof(bio_data,"SSL3 alert %s:%s:%s\n",
			str,
	...
	{
		if (ret == 0)
			infof(bio_data,"%s:failed in %s\n",
			str,SSL_state_string_long(s));
		else if (ret < 0)
		{
			infof(bio_data,"%s:error in %s\n",
				str,SSL_state_string_long(s));
		}
	}
}
                    ...
                    bool *done)
{
  ...
  while(ssl_connect_2 == connssl->connecting_state ||
	...
	// setup detailed ssl log
	bio_data = data;
	SSL_CTX_set_info_callback(connssl->ctx, apps_ssl_info_callback);
	SSL_set_info_callback(connssl->handle, apps_ssl_info_callback);
#endif
	*/
